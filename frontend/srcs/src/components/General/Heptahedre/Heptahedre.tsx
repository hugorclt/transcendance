...
function Heptahedre({ width, height, firstText, secondText }: THeptrahedreProps) {
  return (
    <svg
      ...
      />
      <text font-weight="bold" fontSize={"2em"} fill={COLORS.background} x={width / 2} y={height / 4} text-anchor="middle" alignment-baseline="central">
        {firstText}
      </text>
      <text font-weight="bold" fontSize={"2em"} fill={COLORS.primary} x={width / 2} y={height / 2} text-anchor="middle" alignment-baseline="central">
        {secondText}
      </text>
    </svg>
  );
